...
<div class="w3-panel w3-padding-small w3-card-4 w3-white w3-leftbar w3-border-light-blue" style="min-height:700px;">
  <h1 class="w3-xlarge">Add station supervisor: {{($station)?$station->name:null}}</h1>
	<div class="w3-row w3-panel" style="max-width:100%;">
		@include('w3.components.notification')
		<form class="w3-container" method="POST" action="{{url('/store-station-supervisor/'.$station->uuid)}}">
			@csrf
			<div class="w3-row">
							...
							<select class="w3-select w3-border {{($errors->has('account_id')) ? 'w3-border-red' : 'w3-border-dark-gray'}}" name="account_id">
								...
								@if($accounts)
									@foreach($accounts as $account)
										...
										<option value="{{$account->uuid}}" {{(old('account_id') == $account->uuid)? 'selected':null}}>{{$account->first_name}} {{$account->middle_name}} {{$account->last_name}}</option>
			...
			<div class="w3-row">
				<div class="w3-col w3-padding-small">
					<button class="w3-button w3-large w3-theme w3-hover-light-blue" type="submit" title="Add account supervisory">Add&nbsp;<i class="fa fa-angle-right fa-lg"></i></button>
				</div>
			</div>
		</form>
	</div>
