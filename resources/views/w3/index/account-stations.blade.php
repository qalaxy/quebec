...
<div class="w3-panel w3-padding-small w3-card-4 w3-white w3-leftbar w3-border-light-blue" style="min-height:700px;">
  <h1 class="w3-xlarge">Stations for: {{($account)?$account->first_name:null}}</h1>
	<div class="w3-row w3-panel" style="max-width:100%;">
		<div class="w3-row">
			<a class="w3-button w3-blue w3-hover w3-hover-light-blue" href="{{url('/add-station/'.$account->uuid)}}">ADD</a>
		...
		<div class="w3-responsive w3-white w3-padding-16 w3-text-dark-gray">
			<table class="w3-table-all w3-hoverable">
				...
				@foreach($stns as $stn)
				<tr>
					<td>{{$stn->station()->first()->name}}</td>
					<td>{{date_format(date_create($stn->from), 'd/m/Y')}}</td>
					<td>{{(isset($stn->to)?date_format(date_create($stn->from), 'd/m/Y'):date_format(date_create(today()), 'd/m/Y'))}}</td>
					<td>{{($stn->status)?'Active':'Inactive'}}</td>
					<td><a class="w3-button" href="{{url('edit-account-station/'.$account->uuid.'/'.$stn->uuid)}}" title="Edit {{$stn->station()->first()->name}}"><i class="fa fa-edit fa-lg"></i></a></td>
					<td><button class="w3-button" onclick="deleteStation('{{$account->uuid}}', '{{$stn->uuid}}');" title="Delete {{$stn->station()->first()->name}}">
		...
		{{$stns->links('vendor.pagination.paginator')}}
		@endisset
	</div>
</div>
