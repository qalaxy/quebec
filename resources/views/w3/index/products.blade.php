...
<div class="w3-panel w3-padding-small w3-card-4 w3-white w3-leftbar w3-border-light-blue" style="min-height:700px;">
  <h1 class="w3-xlarge">Products</h1>
	<div class="w3-row w3-panel" style="max-width:100%;">
		<div class="w3-row">
			<a class="w3-button w3-blue w3-hover w3-hover-light-blue" href="{{url('/create-product')}}">CREATE</a>
		...
		<div class="w3-responsive w3-white w3-padding-16 w3-text-dark-gray">
			<table class="w3-table-all w3-hoverable">
				...
				@foreach($products as $product)
					<tr>
						<td><a href="{{url('product/'.$product->uuid)}}" style="text-decoration:none;">{{$product->name}}</a></td>
						<td>{{$product->description}}</td>
						<td><a class="w3-button" href="{{url('edit-product/'.$product->uuid)}}" title="Edit {{$product->name}}"><i class="fa fa-edit fa-lg"></i></a></td>
						<td><button class="w3-button" onclick="deleteProduct('{{$product->uuid}}');" title="Delete {{$product->name}}">
